...
I think, in this case, we need only know that the `geom` has class `"GeomPoint"`. For other geoms, we may need more information, so I propose that we think of this an an object that has a `class` element; we can add other elements for other geoms as needed.
